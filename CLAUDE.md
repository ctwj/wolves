...
- **主题开发回路陷阱**:程序启动时(`infrastructure/support/template/template.go` 的 init)**仅当整个运行目录 `themes/` 不存在**才从 embed 全量释放主题(释放本身是无条件覆盖);`themes/` 已存在则整体跳过——因此改 `main/resources/themes/` 源码、甚至部署新二进制,对已有 `themes/` 的站点都**不生效**。只删 `themes/<name>` 无效且会弄丢主题(守卫判断的是整个 `themes/`,重启后不会补释放);正确做法:要么把源码手动同步到 `themes/<name>/`(Windows 机器上有未跟踪的 `scripts/sync-wolves-theme.ps1`),要么删掉**整个** `themes/` 目录再重启让 embed 重新释放。sync 后 fsnotify 热重载不可靠,建议重启进程。`main/themes/` 本身 git 忽略。
